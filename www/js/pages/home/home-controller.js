...
.controller('HomeCtrl', function ($scope, $ionicPopup,$state, GlobalServices, GlobalConfig, $http, $interval, $ionicLoading, $q) {
  ...
  var reloadHomePage = function () {
    ...
    $q.all([getFirstBorad, getSecondBorad, getShiftOntime]).then(function (responseList) {
      ...
      $scope.sendMessage([summary1,summary2,summary3]);
      if (!homeSliderLoaded) {
  ...
  $scope.sendMessage = function (data) {
    applewatch.sendMessage(data, 'TEST123', function succ() {
      $ionicPopup.alert({
